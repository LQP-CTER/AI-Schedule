...
# Hiện tại (Tháng 5/2025), "Gemini 2.5" chưa phải là model công khai.
# Sử dụng gemini-1.5-flash (cân bằng tốc độ/chi phí) hoặc gemini-1.5-pro (mạnh hơn, có thể chậm/tốn kém hơn).
# Bạn có thể thay đổi model_name thành "gemini-1.5-pro" nếu muốn thử nghiệm.
MODEL_NAME = "gemini-1.5-flash"
try:
    model = genai.GenerativeModel(model_name=MODEL_NAME, generation_config=generation_config)
    st.sidebar.caption(f"Sử dụng model: {MODEL_NAME}")
except Exception as e:
    st.error(f"Lỗi khởi tạo mô hình AI ({MODEL_NAME}): {e}"); st.error("Kiểm tra API Key, tên model và kết nối mạng."); st.stop()
...
def preprocess_pasted_data_for_lookup(df_input):
    ...
    for index, row in df_input.iterrows():
        ...
        for day_index, day_col in day_mapping.items():
            ...
            processed_rows.append({'Date': current_date.date(), 'Employee': employee_name, 'Shift': 'Ca 1', 'Can_Work': can_do_ca1, 'Note': note})
            processed_rows.append({'Date': current_date.date(), 'Employee': employee_name, 'Shift': 'Ca 2', 'Can_Work': can_do_ca2, 'Note': note})
...
def generate_schedule_with_ai(df_input, requirements, model):
    ...
    unique_employee_names = df_input[employee_col].astype(str).str.strip().unique() # Lấy danh sách tên nhân viên duy nhất
    data_prompt_list.append(f"**LƯU Ý QUAN TRỌNG VỀ TÊN NHÂN VIÊN:** Danh sách nhân viên bao gồm: {', '.join(unique_employee_names)}. Mỗi tên là một người riêng biệt. Ví dụ, 'Nguyên' và 'Nguyên Đào' là HAI NGƯỜI KHÁC NHAU. Tuyệt đối không được nhầm lẫn họ.")


    for index, row in df_input.iterrows(): # Format data for prompt
        emp_name = str(row[employee_col]).strip(); data_prompt_list.append(f"Nhân viên: {emp_name}")
        availability_info = []
        if day_cols:
    ...
    req_prompt_list.append(f"- **QUY TẮC XỬ LÝ TÊN NHÂN VIÊN (CỰC KỲ QUAN TRỌNG):** Mỗi tên nhân viên được cung cấp trong dữ liệu (ví dụ: {', '.join(unique_employee_names[:3])}...) phải được coi là một cá nhân HOÀN TOÀN RIÊNG BIỆT. KHÔNG được phép gộp hoặc nhầm lẫn các tên tương tự nhau (ví dụ: 'Nguyên' và 'Nguyên Đào' là hai người khác nhau và phải được xếp lịch độc lập). Hãy xử lý từng dòng dữ liệu nhân viên một cách riêng rẽ.")
    ...
    req_prompt_list.append(f"- **MỤC TIÊU QUAN TRỌNG NHẤT (BẮT BUỘC TUÂN THỦ):** Phân bổ chính xác **{requirements['shifts_per_week_target']} ca làm việc cho MỖI nhân viên** (trừ FM/Sup, hoặc những người có ghi chú 'nghỉ cả tuần' / 'xin nghỉ nguyên tuần' trong cột Ghi Chú, hoặc những người không đăng ký đủ số buổi khả dụng để đạt {requirements['shifts_per_week_target']} ca). Việc này phải được ưu tiên HÀNG ĐẦU, chỉ sau việc tôn trọng các ngày đăng ký 'Nghỉ' cụ thể của nhân viên (ví dụ: 'Nghỉ' trong cột của Thứ 2 thì không xếp lịch cho Thứ 2). Nếu không thể đạt được mục tiêu {requirements['shifts_per_week_target']} ca cho một nhân viên nào đó (mà họ đủ điều kiện), AI PHẢI giải thích rõ ràng lý do cụ thể cho từng trường hợp không đạt được trong phần phản hồi của mình, ngay bên dưới bảng lịch.")
    ...
    req_prompt_list.append(f"  + **Ưu tiên 1 (Bắt buộc):** Nếu cột 'Ghi chú' chứa 'nghỉ cả tuần', 'xin nghỉ nguyên tuần', 'nghỉ', 'bận', 'không thể', 'xin off' -> TUYỆT ĐỐI KHÔNG xếp lịch cho nhân viên đó trong cả tuần (trừ khi ghi chú chỉ rõ phạm vi ngày cụ thể).")
    req_prompt_list.append(f"  + **Ưu tiên 2 (Mong muốn):** Nếu cột 'Ghi chú' chứa 'muốn làm', 'ưu tiên', 'có thể làm' -> CỐ GẮNG xếp nếu không vi phạm ràng buộc khác (mức độ ưu tiên gợi ý: {requirements['preferences_weight_hint']}).")
    req_prompt_list.append(f"  + **Ưu tiên 3 (Giờ làm không trọn vẹn trong cột Ghi chú):** Nếu cột 'Ghi chú' có giờ cụ thể (VD: 'chỉ làm 9h-12h', 'làm từ 16h'), hãy làm theo các bước sau:")
    req_prompt_list.append(f"      1. Ưu tiên xếp đủ số người có thể làm **trọn vẹn** ca đó trước (dựa trên đăng ký các cột ngày).")
    req_prompt_list.append(f"      2. **CHỈ KHI** ca đó vẫn còn thiếu người theo yêu cầu số lượng, thì MỚI xem xét xếp nhân viên có giờ làm không trọn vẹn (theo cột Ghi chú) vào để đáp ứng nguyện vọng của họ (dù họ không làm đủ giờ).")
    req_prompt_list.append(f"      3. Nếu ca đã đủ người làm trọn vẹn, thì KHÔNG xếp thêm người chỉ làm được một phần giờ (theo cột Ghi chú).")
    req_prompt_list.append("- Chỉ xếp lịch vào ca nhân viên đăng ký/có thể làm (dựa trên dữ liệu các cột ngày Thứ 2 - Chủ Nhật).")
...
"""
    ...
    try: # Call AI Model
        st.info(f"⏳ Đang gọi AI ({MODEL_NAME}) để tạo lịch..."); response = model.generate_content(full_prompt)
        st.success(f"✅ AI ({MODEL_NAME}) đã phản hồi."); return response.text
    except Exception as e: st.error(f"Lỗi khi gọi AI ({MODEL_NAME}): {e}"); return None
...
def parse_ai_schedule(ai_response_text):
    ...
    try:
        ...
        if df_schedule.shape[1] > 0 and df_schedule.iloc[:, 0].isnull().all(): # Nếu cột đầu tiên toàn NaN (thường do dấu | ở đầu)
            df_schedule = df_schedule.iloc[:, 1:]
        if df_schedule.shape[1] > 0 and df_schedule.iloc[:, -1].isnull().all(): # Nếu cột cuối cùng toàn NaN (thường do dấu | ở cuối)
            df_schedule = df_schedule.iloc[:, :-1]
        ...
        elif len(df_schedule.columns) == 2 and expected_cols[0].lower() in df_schedule.columns[0].lower() and expected_cols[1].lower() in df_schedule.columns[1].lower():
        ...
        if "Ngày" in df_schedule.columns:
            try:
                ...
                if df_schedule['Ngày'].isnull().any():
                    st.warning("Cảnh báo: Một số giá trị 'Ngày' từ AI không hợp lệ hoặc không đúng định dạng YYYY-MM-DD. Sẽ cố gắng chuyển đổi lại từ định dạng DD/MM/YYYY hoặc MM/DD/YYYY.")
                    for idx, row_data in df_schedule.iterrows():
                        if pd.isna(row_data['Ngày']):
                            try_formats = ['%d/%m/%Y', '%m/%d/%Y', '%Y/%m/%d', '%d-%m-%Y', '%m-%d-%Y']
                            for fmt in try_formats:
                                try:
                                    converted_date = pd.to_datetime(row_data['Ngày_str_backup'], format=fmt, errors='raise')
...
def display_editable_schedule_with_dropdowns(parsed_schedule_df, availability_df):
    ...
    if parsed_schedule_df is None or parsed_schedule_df.empty: st.warning("Không có dữ liệu lịch để hiển thị."); return None # Trả về None nếu không có lịch
    ...
    try:
        ...
        if parsed_schedule_df.empty: st.warning("Không còn dữ liệu hợp lệ sau khi lọc."); return None # Trả về None nếu không có lịch

        # Get unique sorted dates
        unique_dates = sorted(parsed_schedule_df['Ngày'].dt.date.unique())
        if not unique_dates: st.warning("Không có ngày hợp lệ nào trong dữ liệu."); return None # Trả về None nếu không có lịch
        ...
        all_available_employees = [""] + sorted(list(set(availability_df['Employee'].astype(str).str.strip().unique().tolist()))) # Danh sách chung, đảm bảo duy nhất và là string
        ...
        for current_date_obj in unique_dates: # Đổi tên biến để rõ ràng hơn
            ...
            initial_staff_ca1 = [name.strip() for name in staff_ca1_str.split(',') if name.strip() and not "(Thiếu" in name] # Bỏ qua ghi chú thiếu người
            ...
            available_ca1_list_specific = [""] + sorted(list(set(available_ca1_df['Employee'].astype(str).str.strip().unique().tolist())))
            
            options_list_c1 = list(set(available_ca1_list_specific + initial_staff_ca1)) # Gộp danh sách đăng ký và danh sách đã được xếp
            ...
            initial_staff_ca2 = [name.strip() for name in staff_ca2_str.split(',') if name.strip() and not "(Thiếu" in name]
            ...
            available_ca2_list_specific = [""] + sorted(list(set(available_ca2_df['Employee'].astype(str).str.strip().unique().tolist())))
        ...
        with st.container(border=True):
             ...
             if not st.session_state.df_from_paste.empty:
                  if generate_button_placeholder.button("✨ Tạo Lịch với AI", key="generate_ai_button", use_container_width=True):
                      with st.spinner(f"⏳ Đang yêu cầu AI ({MODEL_NAME}) tạo lịch..."):
                          ...
                          if ai_response:
                              ...
                              st.error(f"❌ Không nhận được phản hồi từ AI ({MODEL_NAME}).")
                              st.session_state.edited_schedule_table = create_8_column_df(None) # Tạo bảng trống
